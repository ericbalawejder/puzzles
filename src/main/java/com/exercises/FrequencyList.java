...
public class FrequencyList {
    ...
    static List<String> frequencyList(List<String> words) {
        final Map<String, Integer> wordCount = words.stream()
                .collect(Collectors.groupingBy(Function.identity(),
                        Collectors.reducing(0, value -> 1, Integer::sum)));

        final Comparator<String> frequencyThenLexicographicOrder =
                (w1, w2) -> wordCount.get(w1).equals(wordCount.get(w2)) ?
                        w1.compareTo(w2) :
                        Integer.compare(wordCount.get(w2), wordCount.get(w1));
